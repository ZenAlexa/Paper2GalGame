...
import type { PaperSegment, SegmentType, IncrementalConfig, ParsedPaper, PaperSection, SectionType } from './types';
...
export class PaperSegmentationStrategy {
      ...
      if (mustGeneratePercentage < this.config.minStartPercentage) {
        ...
        for (const segment of sortedSegments) {
          if (!segment.mustGenerateFirst) {
            ...
            const newPercentage = (
              (mustGenerateDialogues + segment.estimatedDialogues) / totalDialogues
            ) * 100;
  ...
  getPrioritySegments(segments: PaperSegment[]): PaperSegment[] {
    return segments.filter(s => s.mustGenerateFirst)
